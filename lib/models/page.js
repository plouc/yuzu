...
function extractUrls(pages, urls, prefix) {
    ...
    pages.forEach(function (page) {
        ...
        if (page.pages.length > 0) {
            extractUrls(page.pages, urls, prefix + page.url);
        }
    });
}

Page.urlsForWebsite = function (id) {
    return Page.forWebsite(id)
        .then(function (pages) {
            var urls = []
            extractUrls(pages, urls);
